...
- DataDistributionResult：
    ...
    - client_train_indices：
        （各客户端本地训练样本的索引。
         是后续 Label Flipping 和 Local Training 的重要输入。
         Label Flipping 只允许修改这些训练样本对应的标签。）

    - client_test_indices：
        （各客户端本地测试样本的索引。
         后续用于本地模型评估，不参与 Label Flipping。）

    - client_sizes：
        （各客户端实际持有的总样本量。
         主要提供给后续 Architecture Allocation 等模块。）

    - client_train_sizes / client_test_sizes：
        （各客户端训练集和本地测试集的实际样本量，
         供后续模块及实验统计使用。）

    - client_label_distribution_train /
      client_label_distribution_test：
        （各客户端 train/test 的标签数量统计。
         主要用于内部检查和实验统计，不作为真实训练数据。）
...
后续主要传递关系：
    ...
    client_train_indices + full_dataset
        -> Label Flipping
        -> Local Training

    client_test_indices + full_dataset
        -> Local Evaluation
...
class DataDistributionResult:
    ...
    这个对象是 Step_01_data_distribution.py 与后续模块之间的主要接口。
